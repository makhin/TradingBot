...
Помимо ADX Trend Following, робот поддерживает дополнительные стратегии:

### MA Crossover (Пересечение скользящих средних)

Классическая стратегия на пересечении быстрой и медленной EMA:

```
✅ Вход Long:  Быстрая EMA пересекла Медленную снизу вверх
✅ Вход Short: Быстрая EMA пересекла Медленную сверху вниз
✅ Выход:      Обратное пересечение или trailing stop
```

**Настройки (appsettings.json → MaStrategy):**
- `FastMaPeriod`: 10 (период быстрой EMA)
- `SlowMaPeriod`: 30 (период медленной EMA)
- `AtrStopMultiplier`: 2.0 (множитель ATR для стопа)
- `TakeProfitMultiplier`: 2.0 (множитель для тейк-профита)

### RSI Mean Reversion (Возврат к среднему по RSI)

Стратегия на экстремальных значениях RSI:

```
✅ Вход Long:  RSI был ниже 30 (oversold) и вернулся выше
✅ Вход Short: RSI был выше 70 (overbought) и вернулся ниже
✅ Выход:      RSI достиг противоположного экстремума или стоп
```

**Настройки (appsettings.json → RsiStrategy):**
- `RsiPeriod`: 14 (период RSI)
- `OversoldLevel`: 30 (уровень перепроданности)
- `OverboughtLevel`: 70 (уровень перекупленности)
- `UseTrendFilter`: true (торговать только по тренду)

---

## Strategy Ensemble (Комбинирование стратегий)

**Зачем это нужно?**

Одна стратегия может давать ложные сигналы. Когда несколько стратегий согласны - вероятность успеха выше!
...
├─────────────────────────────────────────────────────────────┤
│                                                              │
│  ADX Trend Following ──────→ BUY (weight: 40%)               │
│                                   │                          │
│  MA Crossover ─────────────→ BUY (weight: 30%)               │
│                                   │                          │
│  RSI Mean Reversion ───────→ NONE (weight: 30%)              │
│                                   ↓                          │
│                          ╔═══════════════╗                   │
│                          ║  Голосование  ║                   │
│                          ║  BUY: 70%     ║                   │
...
|----------|----------|---------|
| `Enabled` | Использовать Ensemble вместо одной стратегии | false |
| `MinimumAgreement` | Минимальный % согласия для сигнала | 0.6 (60%) |
| `UseConfidenceWeighting` | Учитывать "уверенность" каждой стратегии | true |
| `StrategyWeights` | Веса каждой стратегии | ADX:40%, MA:30%, RSI:30% |
...
[14:00:00] 📊 Weighted scores:
   BUY:  0.4×0.75 + 0.3×0.60 = 0.48 (48%)
   NONE: 0.3×1.0 = 0.30 (30%)

[14:00:00] ✅ Consensus NOT reached (48% < 60%)
[14:00:00] ⏸️ No trade - waiting for stronger consensus
```
...
[18:00:00] 📊 Weighted scores:
   BUY: 0.4×0.85 + 0.3×0.80 + 0.3×0.65 = 0.775 (77.5%)

[18:00:00] ✅ Consensus reached: BUY (77.5% >= 60%)
[18:00:00] 📈 Opening long position...
```

**Преимущества Ensemble:**

✅ **Меньше ложных сигналов**: Требуется согласие нескольких стратегий
✅ **Диверсификация**: Разные стратегии ловят разные паттерны
✅ **Адаптивность**: Если одна стратегия "сломалась", другие компенсируют
✅ **Гибкость**: Легко добавить/убрать стратегии и настроить веса

**Недостатки:**

⚠️ **Меньше сделок**: Ждём консенсус → реже входим
⚠️ **Пропуски**: Можем пропустить хорошие движения
⚠️ **Сложность**: Больше параметров для настройки

**Когда использовать:**

- ✅ Когда хотите более консервативную торговлю
- ✅ Когда одна стратегия даёт много ложных сигналов
- ✅ Когда торгуете на нескольких инструментах
- ❌ Когда хотите максимум сделок
- ❌ Когда торгуете очень волатильный актив

**Код для использования:**

```csharp
// Создать ensemble с дефолтными стратегиями
...
// Или добавить стратегии вручную
...
// Посмотреть как голосовали стратегии
...
## Режим 2.5: Генетический оптимизатор (Эволюционный поиск)

### Что это?

Генетический алгоритм — это умный способ поиска лучших параметров,
вдохновлённый биологической эволюцией. Вместо перебора **всех** комбинаций
(что занимает много времени), алгоритм "эволюционирует" популяцию решений.

```
┌────────────────────────────────────────────────────────────────┐
│              ГЕНЕТИЧЕСКИЙ АЛГОРИТМ vs GRID SEARCH              │
├────────────────────────────────────────────────────────────────┤
│                                                                │
│  Grid Search (перебор):           Генетический алгоритм:       │
│  ┌─┬─┬─┬─┬─┬─┬─┬─┬─┬─┐          ┌─────────────────────┐       │
│  │●│●│●│●│●│●│●│●│●│●│          │ Поколение 1:        │       │
│  │●│●│●│●│●│●│●│●│●│●│          │   ◆ ◆ ◆ ◆ ◆ (случ.) │       │
│  │●│●│●│●│●│●│●│●│●│●│          │ Поколение 5:        │       │
│  │●│●│●│●│●│●│●│●│●│●│          │     ◆ ◆ ◆ (лучшие)  │       │
│  └─┴─┴─┴─┴─┴─┴─┴─┴─┴─┘          │ Поколение 20:       │       │
│  Проверяет ВСЕ точки            │       ★ (оптимум!)  │       │
│  = медленно                      └─────────────────────┘       │
│                                  Эволюционирует к лучшему      │
│                                  = быстро                      │
└────────────────────────────────────────────────────────────────┘
```
...
│                                                                 │
│  1. ИНИЦИАЛИЗАЦИЯ                                               │
│     Создаём 100 случайных наборов параметров ("особей")        │
│                                                                 │
│  2. ОЦЕНКА ПРИСПОСОБЛЕННОСТИ                                    │
│     Тестируем каждый набор на исторических данных              │
│     Fitness = Sharpe Ratio / Profit Factor / etc.              │
│                                                                 │
│  3. ОТБОР (Selection)                                           │
│     Лучшие "выживают" и размножаются                           │
│     Турнирный отбор: сравниваем 5 случайных, берём лучшего     │
│                                                                 │
│  4. СКРЕЩИВАНИЕ (Crossover)                                     │
│     Комбинируем параметры двух "родителей"                     │
│     Родитель A: ADX=14, EMA=20/50                              │
│     Родитель B: ADX=10, EMA=15/60                              │
│     Потомок:    ADX=14, EMA=15/50 (микс)                       │
│                                                                 │
│  5. МУТАЦИЯ (Mutation)                                          │
│     Случайно изменяем некоторые параметры                      │
│     15% шанс мутации → добавляет разнообразие                  │
│                                                                 │
│  6. ПОВТОРЯЕМ поколения 2-5 пока не найдём оптимум             │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

### Преимущества генетического оптимизатора:

```
┌────────────────────────────────────────────────────────────────┐
│ ✅ В 10-100 раз быстрее полного перебора                       │
│ ✅ Находит глобальный оптимум (а не локальный)                 │
│ ✅ Автоматически избегает переобучения (overfitting)           │
│ ✅ Встроенная валидация на out-of-sample данных                │
│ ✅ Параллельная обработка (использует все ядра CPU)            │
│ ✅ Ранняя остановка при достижении плато                       │
└────────────────────────────────────────────────────────────────┘
```
...
"GeneticOptimizer": {
  ...
  "EliteCount": 5,              // Сколько лучших сохранять без изменений
  "TournamentSize": 5,          // Размер турнира для отбора
  "CrossoverRate": 0.8,         // Вероятность скрещивания (80%)
  "MutationRate": 0.15,         // Вероятность мутации (15%)
  "EarlyStoppingPatience": 10,  // Остановка если нет улучшений 10 поколений
  "FitnessFunction": "RiskAdjusted", // Критерий оптимизации
  "MinTrades": 30,              // Минимум сделок для валидности
  "TrainingRatio": 0.7,         // 70% данных на обучение, 30% на валидацию
  "ParameterRanges": {
...
### Функции приспособленности (Fitness Functions):

```
┌─────────────────┬──────────────────────────────────────────────┐
│ FitnessFunction │ Описание                                     │
├─────────────────┼──────────────────────────────────────────────┤
│ Sharpe          │ Доходность с учётом волатильности            │
│ Sortino         │ Как Sharpe, но учитывает только убытки       │
│ ProfitFactor    │ Прибыль / Убытки (>1.5 хорошо)               │
│ Return          │ Максимальная доходность                      │
│ RiskAdjusted    │ Sharpe с штрафом за просадку (рекомендуется) │
│ Combined        │ Sharpe × ProfitFactor × (1-MaxDD/100)        │
└─────────────────┴──────────────────────────────────────────────┘
```
...
│ ATR Multiplier    │ 2.3                                    │
│ Take Profit ×     │ 1.8                                    │
│ Volume Threshold  │ 1.4                                    │
├───────────────────┼────────────────────────────────────────┤
│ Training Fitness  │ 1.82 (Sharpe)                          │
│ Validation Fit.   │ 1.65 (Sharpe)                          │
│ Robustness        │ 90.7%  ✅ (>50% = надёжно)             │
└───────────────────┴────────────────────────────────────────┘

┌────────────────────────────────────────────────────────────┐
│                  TOP 5 ROBUST RESULTS                      │
├───┬───────┬───────────┬────────────┬──────────────────────┤
│ # │ Train │ Validate  │ Robustness │ Parameters           │
├───┼───────┼───────────┼────────────┼──────────────────────┤
│ 1 │ 1.82  │ 1.65      │ 90.7% ✅   │ ADX=14 EMA=18/52     │
│ 2 │ 1.76  │ 1.58      │ 89.8% ✅   │ ADX=14 EMA=20/50     │
│ 3 │ 1.71  │ 1.52      │ 88.9% ✅   │ ADX=12 EMA=16/55     │
│ 4 │ 1.68  │ 1.42      │ 84.5% ✅   │ ADX=16 EMA=22/48     │
│ 5 │ 1.65  │ 1.38      │ 83.6% ✅   │ ADX=14 EMA=18/60     │
└───┴───────┴───────────┴────────────┴──────────────────────┘
```
...
1. НАЧНИТЕ С ДЕФОЛТНЫХ НАСТРОЕК
   PopulationSize=100, Generations=50 — хороший баланс

2. ВЫБЕРИТЕ ПРАВИЛЬНУЮ FITNESS FUNCTION
   - RiskAdjusted — для консервативной торговли
   - ProfitFactor — если важна стабильность
   - Combined — универсальный вариант

3. ПРОВЕРЯЙТЕ ROBUSTNESS
   Robustness >= 50% означает, что параметры работают
   не только на обучающих данных

4. НЕ ПЕРЕУСЕРДСТВУЙТЕ
   Если Generations > 100 и улучшений нет — это переобучение

5. ИСПОЛЬЗУЙТЕ РАЗНЫЕ ПЕРИОДЫ ДАННЫХ
   Оптимизируйте на 2-3 годах, проверяйте на свежих данных
```
...
WebSocket соединение с Binance может обрываться по разным причинам:
- Проблемы с интернетом
- Технические работы на бирже
- Сетевые сбои
- Ограничения провайдера

**Без Reconnection Logic:**
- 😱 Бот останавливается при потере связи
- 😱 Пропускает важные ценовые движения
- 😱 Не может закрыть позицию по стопу/профиту
- 😱 Требует ручного перезапуска

**ConnectionManager решает эту проблему** - автоматически переподключается с exponential backoff.
...
connectionManager.OnDisconnected += (reason) =>
    Console.WriteLine($"⚠️ Disconnected: {reason}");

connectionManager.OnError += (ex) =>
    Console.WriteLine($"❌ Error: {ex.Message}");

// Подключиться с автоматическим переподключением
...
// Запустить health check (опционально)
...
При потере соединения бот пытается переподключиться с растущими задержками:

```
Попытка 1: через 1 секунду    (1s)
Попытка 2: через 2 секунды    (2s)
Попытка 3: через 4 секунды    (4s)
Попытка 4: через 8 секунд     (8s)
Попытка 5: через 16 секунд    (16s)
Попытка 6: через 32 секунды   (32s)

Если все попытки провалились → бот останавливается
```

**Зачем растущие задержки?**
- Не перегружать сервер при проблемах на бирже
- Дать время сети восстановиться
...
```
[14:30:00] ✅ Connected to BTCUSDT 4h stream
[14:45:00] ⚠️ WebSocket connection lost
[14:45:00] 🔄 Starting automatic reconnection...
[14:45:00] 🔌 Connecting to BTCUSDT 4h stream (attempt 1/6)...
[14:45:01] ✅ Connected to BTCUSDT 4h stream
[14:45:01] 💚 Reconnect successful on first attempt!
```

**Сценарий 2: Проблемы с интернетом (несколько попыток)**

```
[15:00:00] ⚠️ WebSocket connection lost
[15:00:00] 🔄 Starting automatic reconnection...
[15:00:00] 🔌 Connecting to BTCUSDT 4h stream (attempt 1/6)...
[15:00:01] ❌ Connection failed: No internet connection
[15:00:01] ⏳ Retrying in 1000ms... (attempt 1/6)

[15:00:02] 🔌 Connecting to BTCUSDT 4h stream (attempt 2/6)...
[15:00:03] ❌ Connection failed: Timeout
[15:00:03] ⏳ Retrying in 2000ms... (attempt 2/6)

[15:00:05] 🔌 Connecting to BTCUSDT 4h stream (attempt 3/6)...
[15:00:06] ❌ Connection failed: Timeout
[15:00:06] ⏳ Retrying in 4000ms... (attempt 3/6)

[15:00:10] 🔌 Connecting to BTCUSDT 4h stream (attempt 4/6)...
[15:00:11] ✅ Connected to BTCUSDT 4h stream
[15:00:11] 💚 Reconnected after 4 attempts (11 seconds downtime)
```

**Сценарий 3: Длительный сбой (все попытки провалились)**

```
[16:00:00] ⚠️ WebSocket connection lost
[16:00:00] 🔄 Starting automatic reconnection...

[Попытки 1-6 с exponential backoff...]

[16:01:03] ❌ Max reconnection attempts reached (6)
[16:01:03] ⚠️ Bot will stop - manual intervention required
[16:01:03] 💡 Recommendation: Check internet connection and restart bot
```

**Сценарий 4: Соединение восстановилось автоматически**

```
[17:00:00] ⚠️ WebSocket connection lost
[17:00:15] ✅ WebSocket connection restored (downtime: 15.2s)
[17:00:15] 💚 Connection restored automatically by Binance.Net
[17:00:15] ℹ️ No reconnection attempts needed
```
...
```
[18:00:00] 💚 Starting health check (interval: 300s)
[18:05:00] 💚 Health check: Connected
[18:10:00] 💚 Health check: Connected
[18:15:00] 💔 Health check: DISCONNECTED
[18:15:00] ⚠️ Health check detected disconnection
[18:15:00] 🔄 Triggering reconnection...
```

**Интеграция с BinanceLiveTrader:**
...
public class BinanceLiveTrader
{
    ...
    public BinanceLiveTrader(...)
    {
        ...
        {
            Log($"⚠️ Stream disconnected: {reason}");
            // Можно отправить Telegram alert
        };
    ...
    public async Task StartAsync()
    {
        // Использовать ConnectionManager вместо прямого подключения
...
| `OnDisconnected` | Потеря соединения | `string reason` |
| `OnError` | Ошибка при подключении | `Exception ex` |
| `OnLog` | Любое логирование | `string message` |

**Преимущества:**

1. **Автоматическое восстановление:**
   - Не требует ручного вмешательства
   - Exponential backoff защищает от перегрузки
   - Максимум 6 попыток за ~63 секунды

2. **Прозрачность:**
   - Детальное логирование всех попыток
   - События для интеграции с уведомлениями
   - Статистика соединения

3. **Надёжность:**
   - Health check для раннего обнаружения проблем
   - Graceful обработка всех сценариев
   - Корректное закрытие при остановке

**Важные моменты:**

⚠️ **Позиции остаются открытыми:**
- Даже при потере связи, OCO ордера на бирже продолжают работать
- Stop-loss защитит вас даже если бот отключён
- После переподключения бот продолжит мониторинг

⚠️ **Максимум 6 попыток:**
- Если все попытки провалились - бот останавливается
- Это защита от бесконечных попыток при серьёзных проблемах
- Требуется ручной перезапуск и проверка подключения

⚠️ **Health check - опциональный:**
- Не обязателен, Binance.Net сам обнаруживает потерю связи
- Полезен для дополнительного мониторинга
- Рекомендуемый интервал: 5-10 минут

**FAQ:**

**Q: Что происходит с позициями при потере связи?**
A: Ничего! OCO ордера на бирже продолжают работать независимо от бота. Stop-loss и take-profit остаются активными.

**Q: Сколько времени занимает переподключение?**
A: От 1 секунды (если связь сразу восстановилась) до ~63 секунд (если использованы все 6 попыток).

**Q: Можно ли изменить количество попыток?**
A: Да, измените массив `_backoffDelays` в коде ConnectionManager.

**Q: Нужно ли запускать health check?**
A: Не обязательно. Binance.Net сам обнаруживает потерю связи. Health check - дополнительная страховка.

**Q: Что делать если все попытки провалились?**
A:
1. Проверьте интернет-соединение
2. Проверьте статус Binance (status.binance.com)
3. Перезапустите бота вручную
4. Проверьте позиции на бирже

**Q: Можно ли получать Telegram уведомления о переподключениях?**
A: Да! Подпишитесь на события `OnDisconnected` и `OnConnected` и отправляйте уведомления через TelegramNotifier.

**Q: Что происходит когда все 6 попыток провалились?**
A: Срабатывает событие `OnCriticalFailure` - это критическая ситуация, требующая немедленного внимания. Рекомендуется отправить уведомление в Telegram.

---

### Критические уведомления в Telegram

**Когда бот полностью отвалился - критически важно узнать об этом немедленно!**

ConnectionManager генерирует событие `OnCriticalFailure` когда:
- Исчерпаны все 6 попыток переподключения
- Автоматическое переподключение провалилось
- Бот больше не может продолжать работу
...
public class BinanceLiveTrader
{
    ...
    public BinanceLiveTrader(...)
    {
        ...
        {
            var message = "🚨 КРИТИЧЕСКАЯ ОШИБКА!\n\n" +
                         "❌ Бот не может подключиться к Binance\n" +
                         "❌ Все попытки переподключения провалились\n" +
                         "⚠️ Требуется немедленное вмешательство!\n\n" +
                         "Действия:\n" +
                         "1. Проверьте интернет-соединение\n" +
                         "2. Проверьте статус Binance (status.binance.com)\n" +
                         "3. Проверьте открытые позиции на бирже\n" +
                         "4. Перезапустите бота вручную\n\n" +
                         "⚡ OCO ордера на бирже продолжают работать!";

            await _telegram?.SendMessageAsync(message);

            Log("🚨 Critical failure notification sent to Telegram");
        };
        ...
        _connectionManager.OnDisconnected += async (reason) =>
        {
            await _telegram?.SendMessageAsync(
                $"⚠️ Потеря связи с Binance\n" +
                $"Причина: {reason}\n" +
                $"🔄 Начинаю автоматическое переподключение..."
            );
        };
        ...
        _connectionManager.OnConnected += async () =>
        {
            if (_connectionManager.ReconnectAttempt > 0)
            {
                await _telegram?.SendMessageAsync(
                    $"✅ Связь восстановлена!\n" +
                    $"Попыток: {_connectionManager.ReconnectAttempt}\n" +
...
**Пример критического сценария с Telegram:**

```
[20:00:00] ⚠️ WebSocket connection lost
[20:00:00] 📱 Telegram: "⚠️ Потеря связи с Binance. Начинаю переподключение..."

[20:00:00] 🔌 Connecting to BTCUSDT 4h stream (attempt 1/6)...
[20:00:01] ❌ Connection failed: Timeout
[20:00:01] ⏳ Retrying in 1000ms...

[20:00:02] 🔌 Connecting to BTCUSDT 4h stream (attempt 2/6)...
[20:00:03] ❌ Connection failed: Timeout
[20:00:03] ⏳ Retrying in 2000ms...

[20:00:05] 🔌 Connecting to BTCUSDT 4h stream (attempt 3/6)...
[20:00:06] ❌ Connection failed: Timeout
[20:00:06] ⏳ Retrying in 4000ms...

[20:00:10] 🔌 Connecting to BTCUSDT 4h stream (attempt 4/6)...
[20:00:11] ❌ Connection failed: Timeout
[20:00:11] ⏳ Retrying in 8000ms...

[20:00:19] 🔌 Connecting to BTCUSDT 4h stream (attempt 5/6)...
[20:00:20] ❌ Connection failed: Timeout
[20:00:20] ⏳ Retrying in 16000ms...

[20:00:36] 🔌 Connecting to BTCUSDT 4h stream (attempt 6/6)...
[20:00:37] ❌ Connection failed: Timeout
[20:00:37] ⏳ Retrying in 32000ms...

[20:01:09] ❌ Max reconnection attempts reached (6)
[20:01:09] ⚠️ Bot will stop - manual intervention required
[20:01:09] 💡 Recommendation: Check internet connection and restart bot
...
✅ **OCO ордера продолжают работать:**
- Даже если бот полностью отвалился
- Stop-loss защитит ваши позиции
- Take-profit сработает если цена дойдёт
- Ордера на бирже независимы от бота

✅ **Немедленное уведомление:**
- Вы узнаете о проблеме сразу
- Сможете проверить позиции вручную
- Примете решение о дальнейших действиях

✅ **Детальная информация:**
- Количество попыток переподключения
- Причина каждого отказа
- Рекомендации по устранению

**События ConnectionManager (обновлённая таблица):**

| Событие | Когда срабатывает | Параметры | Telegram |
|---------|------------------|-----------|----------|
| `OnConnected` | Успешное подключение | - | ✅ Уведомить о восстановлении |
| `OnDisconnected` | Потеря соединения | `string reason` | ⚠️ Уведомить о потере связи |
| `OnError` | Ошибка при подключении | `Exception ex` | ℹ️ Опционально |
| `OnLog` | Любое логирование | `string message` | ❌ Не нужно |
| `OnCriticalFailure` | Все попытки провалились | - | 🚨 **КРИТИЧЕСКИ ВАЖНО!** |

**Рекомендуемая стратегия уведомлений:**

1. **OnDisconnected** → Информационное сообщение
   - Не паникуйте, это может быть кратковременный сбой
   - Бот автоматически попытается переподключиться

2. **OnConnected (после переподключения)** → Успокаивающее сообщение
   - Связь восстановлена
   - Указать сколько попыток потребовалось

3. **OnCriticalFailure** → Критическое уведомление
   - Яркое сообщение с emoji 🚨
   - Чёткие инструкции что делать
   - Напоминание что OCO ордера работают
...
При остановке бота (Ctrl+C или закрытие терминала) важно:
- Сохранить текущее состояние (позиции, equity, OCO ордера)
- Дать возможность закрыть позиции или оставить их с защитой
- Уведомить о завершении работы

**Без Graceful Shutdown:**
- 😱 Состояние не сохраняется
- 😱 Неясно что происходит с позициями
- 😱 Нет уведомления о том что бот остановлен
- 😱 При перезапуске бот не знает о текущих позициях

**GracefulShutdown решает эту проблему** - корректно завершает работу с сохранением состояния.
...
try
{
    // Запустить бота с токеном отмены
    await trader.StartAsync(shutdown.Token);
}
catch (OperationCanceledException)
{
    // Нормальное завершение через Graceful Shutdown
...
🛑 Initiating graceful shutdown: Ctrl+C pressed
💾 Saving current state...
💾 State saved: 1 positions, Equity: $10,245.50
...
📱 Telegram: "🛑 Bot shutdown: Ctrl+C pressed
📊 Open positions: 0"
```

**Сценарий 1: Оставить позицию открытой (вариант по умолчанию)**

```
Your choice [1]: 1

✅ Positions kept open with OCO protection

[Bot stops]

📱 Telegram: "🛑 Bot shutdown: Ctrl+C pressed
📊 Open positions: 1
ℹ️ OCO orders remain active on exchange"
```

**Что происходит:**
- ✅ Состояние сохранено в bot_state.json
- ✅ OCO ордер продолжает работать на бирже
- ✅ Stop-loss защищает позицию
- ✅ Take-profit сработает если цена дойдёт
- ✅ При перезапуске бот восстановит состояние

**Сценарий 2: Закрыть позиции по рынку**
...
**Что происходит:**
- ✅ Все позиции закрыты по рыночной цене
- ✅ OCO ордера автоматически отменены
- ✅ Состояние сохранено (нет открытых позиций)
- ✅ Можно спокойно перезапускать бота

**Сценарий 3: Закрыть позиции и отменить OCO**

```
Your choice [1]: 3

📤 Closing all positions...
✅ OCO Order cancelled (ID: 123456789)
[16:30:05] Market order placed: BTCUSDT SELL 0.05000
  ✅ Closed BTCUSDT @ 42,150.00
✅ All positions closed
...
**Что происходит:**
- ✅ OCO ордера явно отменены
- ✅ Позиции закрыты по рынку
- ✅ Полная очистка всех ордеров на бирже

**Обработка системного завершения (ProcessExit):**

Если процесс завершается не через Ctrl+C (например, kill, системное выключение):

```
[System sends termination signal]

🛑 Initiating graceful shutdown: Process exit
💾 Saving current state...
💾 State saved: 1 positions, Equity: $10,245.50
...
1. **Timeout 30 секунд** для ProcessExit:
   - Система даёт 30 секунд на корректное завершение
   - За это время бот сохраняет состояние
   - Если не успеет - процесс завершится принудительно

2. **Интерактивный режим**:
   - Если консоль интерактивная - спрашивает пользователя
   - Если неинтерактивная (systemd, docker) - автоматически оставляет позиции открытыми

3. **Защита от повторного вызова**:
   - Если Shutdown уже начался, повторные вызовы игнорируются
   - Предотвращает конфликты при множественных сигналах
...
if (savedState != null)
{
    Console.WriteLine($"📂 Found saved state from {savedState.LastUpdate}");
    ...
    {
        Console.WriteLine("✅ State fully reconciled with exchange");
        // Восстановить позиции в RiskManager
    }
    else
    {
        Console.WriteLine("⚠️ State mismatch detected - manual review required");
    }
}
```

**Пример сохранённого состояния (bot_state.json):**
...
GracefulShutdown автоматически:
- ✅ Отменяет CancellationToken (останавливает торговлю)
- ✅ Сохраняет состояние через StateManager
- ✅ Останавливает трейдер (закрывает WebSocket)
- ✅ Отправляет Telegram уведомление
- ✅ Логирует все действия

**Тестирование Graceful Shutdown:**

1. **Запустите бота в paper trading:**
   ```
   dotnet run -- live
   Symbol: BTCUSDT
   Paper trading: Yes
...
2. **Откройте позицию** (дождитесь сигнала или используйте testnet)

3. **Нажмите Ctrl+C:**
   - Проверьте что состояние сохранено
   - Убедитесь что файл bot_state.json создан
   - Проверьте Telegram уведомление

4. **Перезапустите бота:**
   - Убедитесь что состояние загружено
   - Проверьте что позиция восстановлена

**Рекомендации:**

✅ **Всегда используйте Graceful Shutdown:**
```csharp
...
✅ **Для production:**
- Вариант 1 (оставить позиции) - если доверяете OCO ордерам
- Вариант 3 (закрыть всё) - если хотите полного контроля

✅ **Для неинтерактивных сервисов (systemd, docker):**
- Позиции автоматически остаются открытыми
- OCO ордера продолжают защищать
- State сохраняется для восстановления

⚠️ **Важно:**
- Graceful Shutdown имеет timeout 30 секунд
- За это время должны завершиться все операции
- Если timeout истёк - процесс завершится принудительно

**FAQ:**

**Q: Что происходит с позициями если я выбрал "Keep positions open"?**
A: OCO ордера остаются активными на бирже независимо от состояния бота. Stop-loss и take-profit продолжают работать.

**Q: Безопасно ли закрывать позиции по рынку (вариант 2)?**
A: Да, но возможно проскальзывание. Если рынок волатилен, лучше оставить OCO ордера.

**Q: Можно ли восстановить позиции после перезапуска?**
A: Да! StateManager сохраняет все позиции в bot_state.json. При перезапуске используйте LoadState() и ReconcileWithExchange().

**Q: Что если бот крашится без Graceful Shutdown?**
A: OCO ордера на бирже продолжат работать, но состояние не сохранится. При следующем запуске используйте ReconcileWithExchange() для проверки позиций.

**Q: Работает ли Graceful Shutdown в Docker/systemd?**
A: Да! При неинтерактивном режиме автоматически выбирается вариант 1 (keep positions open).

**Q: Сколько времени занимает Graceful Shutdown?**
A: Обычно 1-3 секунды. При закрытии позиций - до 10 секунд в зависимости от скорости биржи.

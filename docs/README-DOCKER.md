...
chmod 777 data/logs  # Разрешить запись логов из контейнера
```

> **Примечание:** Директория `data/logs` должна быть доступна для записи, так как приложение внутри контейнера работает от имени непривилегированного пользователя `tradingbot`.

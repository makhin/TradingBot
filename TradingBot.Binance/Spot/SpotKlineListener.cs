...
/// <summary>
/// Binance Spot WebSocket kline/candle listener implementation
/// </summary>
public class SpotKlineListener : IKlineListener
{
    ...
    public SpotKlineListener(BinanceSocketClient socketClient, ILogger? logger = null)
    {
        _socketClient = socketClient;
        _logger = logger ?? Log.ForContext<SpotKlineListener>();
    }
        ...
        CancellationToken ct = default)
    {
        ...
        _logger.Information("Subscribing to kline updates: {Symbol} {Interval}", symbol, binanceInterval);
        ...
        if (!result.Success)
        {
            _logger.Error("Failed to subscribe to klines: {Error}", result.Error?.Message);
            return null;
        }

        _currentSubscription = result.Data;
        _logger.Information("Successfully subscribed to kline updates");

        return new SubscriptionWrapper(result.Data, () =>
        {
            _currentSubscription = null;
            _logger.Information("Unsubscribed from kline updates");
        });
    }
    ...
    public async Task UnsubscribeAllAsync()
    {
        if (_currentSubscription != null)
        {
            ...
            _currentSubscription = null;
            _logger.Information("Closed all kline subscriptions");
        }
    }
